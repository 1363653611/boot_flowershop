...
public class OrdermsgController extends BaseController {
	...
	public String faHuo(int id,HttpServletRequest request){
		...
		if(flag==true){
			...
			if(ordermsg.getShfs().equals("自取")){
				ordermsg.setFhstatus("已发货");
			ordermsg.setShstatus("已收货");
			}else{
				ordermsg.setFhstatus("已发货");
				ordermsg.setShstatus("");
			}
	...
	//货到付款
		...
		public String orderhdfk(int id, HttpServletRequest request,RedirectAttributes redirectAttributes){
			Member member = (Member)request.getSession().getAttribute("sessionmember");
			if(member!=null){
				List<Cart> cartlist = cartDAO.selectMyProductList(member.getId());
				String totalstr = "";
			...
			return "redirect:orderList.do";
		    	...
		    	redirectAttributes.addFlashAttribute("message","余额不足");
		    return "redirect:orderList.do";
		    }
