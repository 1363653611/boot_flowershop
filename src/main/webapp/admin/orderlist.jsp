      ...
      <table class="table table-striped b-t b-light">
        ...
        <c:forEach items="${list}" var="order" varStatus="status">
          <tr>
			...
			<c:choose>
			  <c:when test="${order.shfs eq '送货上门'}">
			...
			<c:if test="${order.shfs eq '送货上门' and order.fhstatus eq '待发货' and order.shstatus eq ''}">
			    <a class="layui-btn layui-btn-mini" href="faHuo.do?id=${order.id}"><i class="iconfont icon-edit"></i> 确认发货</a>
			</c:if>
            
            <c:if test="${order.shfs eq '送货上门' and order.fhstatus eq '已发货' and order.shstatus eq ''}">
              <a href="logisticsadd.jsp?orderno=${order.orderno}" title="发往" class="active" ui-toggle-class=""><i>添加物流信息</i></a>
            </c:if>
